...
1. 用户已确认文号为 `财金〔2017〕90号`，但该确认尚未写入某个正式任务的审核元数据，且 PDF 内没有页码定位；审核页仍需保存人工确认和审计记录。
...
5. 当前地址是本机回环地址，不是公网生产地址；公网后端、外部数据库/队列、备份恢复演练和正式监控仍需在目标部署环境完成。
...
1. 在审核页保存用户已确认的文号 `财金〔2017〕90号`，并保留人工确认审计记录。
