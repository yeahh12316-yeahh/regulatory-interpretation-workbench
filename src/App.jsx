...
function ReviewPanel({ session, taskId, onClose, onReviewChanged, notify }) {
  ...
  const [metadata, setMetadata] = useState({ document_no: '', issuer: '', publish_date: '', effective_date: '', attachment_resolution: '', attachment_note: '' })
  ...
  async function loadReview() {
    ...
    try {
      ...
      setMetadata({
        ...
        attachment_note: nextReview.task?.processing_config?.review_overrides?.attachment_note || '',
      })
  ...
  async function saveMetadata(event) {
    ...
    try {
      const nextReview = await apiClient.updateReviewMetadata(taskId, {
        ...
        attachment_note: metadata.attachment_note.trim() || null,
      }, session?.accessToken)
      setReview(nextReview)
      onReviewChanged(nextReview)
  ...
  const qcSummary = getLatestReviewGateSummary(review.task, review.qc_results)
  const blockers = qcSummary.blockers
  const qcStatusLabel = qcSummary.status === 'blocked' ? '已阻断' : qcSummary.status === 'passed' ? '已通过' : '待检查'
  const targetLabels = { task: '任务', source_document: '来源文件', requirement: '监管要求', interpretation: '条款解读', evidence: '证据' }
