...
**Goal:** 建设一个面向银行、保险、证券、基金、期货、信托、金融租赁、支付、消费金融、财务公司和小额贷款机构的真实可用外规解读 Agent 网站，实现“选择机构类型 → 上传法规 → 条款解析 → 外规解读 → 证据追踪 → HTML 阅读 → Word 导出”的完整闭环。
...
3. 完成网站信息架构、用户流程、页面线框、视觉系统和交互规范。
4. 完成 HTML 阅读页、Word 报告和新旧规比较结果的样式设计。
